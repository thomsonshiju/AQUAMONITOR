...
import { Users, Bell, Shield, Send, CheckCircle, Info, AlertCircle, AlertTriangle, Trash2 } from 'lucide-react';
import { db } from '../firebaseConfig';
import { collection, addDoc } from 'firebase/firestore';

// Broadcast Notification Panel Component
function BroadcastNotificationPanel({ users }) {
    ...
    const handleSendNotification = async (e) => {
        ...
        try {
            ...
            // Using a batched approach or Promise.all for parallelism
    ...
    return (
        <div className="fade-in">
            ...
            <div style={{ display: 'grid', gap: '2rem' }}>
                {/* Admin Login Info (The "Admin Login" view) */}
                <div className="card" style={{ background: 'linear-gradient(135deg, var(--bg-card), rgba(99, 102, 241, 0.05))', borderColor: 'var(--secondary)' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        ...
                        <div>
                            ...
                            {user?.phone && (
                                <div style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>{user?.phone}</div>
                            )}
                            <div style={{ color: 'var(--secondary)', fontSize: '0.8rem', marginTop: '0.2rem', fontWeight: 500 }}>Logged in as Administrator</div>
                        </div>
                    </div>
                </div>
                {/* User Management */}
                ...
                <div className="card">
                    <h3 style={{ marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <Bell size={20} /> Broadcast Notifications
                    </h3>

                    <BroadcastNotificationPanel users={users} />
                </div>
            </div>
        </div>
    );
